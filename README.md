...
-사용한기술과구현한기능:
  ...
  ![실행화면](img/screen7.png)
  ![실행화면](img/screen8.png)

  -구현한내용(위그림참조)
   -파일 열기 기능 구현: 사용자가 저장된 이미지 파일을 열어서 편집할 수 있는 기능을 구현한다. (예: PNG, JPG, BMP 형식)
   -파일 열기를 위한 대화상자인 OpenFileDialog 사용
   -파일 열기 완료후 사진 위에 그림 그리기 기능 구현
   -파일 열기 후 확대/축소 기능 추가
   -열린 파일에 확대/축소 후 그림 그리기 기능 구현
   -파일 확대 후 상하/좌우 스크롤 바를 구현하여 스크롤시 확대되어 보이지 않던 부분이 보이게 구현
   -처음 이미지 파일 열기시 사진이 알아서 PictureBox 크기에 맞게 조절되어 보이도록 구현
   -파일을 열고 그림을 그린 후 저장시 기존 파일 저장과 마찬가지로 (png, jpg, bmp 형식)으로 저장 가능하도록 구현
